...
<div class="block_keyboard--list4 list">
    ...
    <button type="button" value="KeyZ"></button>
...
if (localStorage.getItem('language') === null || localStorage.getItem('language') === 'en') {
    for(elem of buttonCollection) {
        ...
        if(keyStorage[elem.value]["keyEn"].length !== 1) {
            elem.innerHTML = keyStorage[elem.value]["keyShiftEn"];
        } else {
            elem.innerHTML = keyStorage[elem.value]["keyEn"];
        }
    }
} else if (localStorage.getItem('language') === 'ru') {
    for(elem of buttonCollection) {
        if(keyStorage[elem.value]["keyEn"].length !== 1) {
            elem.innerHTML = keyStorage[elem.value]["keyShiftRu"];
        } else {
            elem.innerHTML = keyStorage[elem.value]["keyRu"];
        }
    }
}
            ...
            if (e.code === elem.value) {
                if (caseType === 'lower') {
                    if (localStorage.getItem('language') === 'en') {
                        textarea.value += keyStorage[elem.value]['keyShiftEn'];
                    } else if (localStorage.getItem('language') === 'ru') {
                        textarea.value += keyStorage[elem.value]['keyShiftRu'];
                    }
                } else if(caseType === 'upper') {
                    if (localStorage.getItem('language') === 'en') {
                        if (elem.value.slice(0, 3) === "Key") {
                            textarea.value += keyStorage[elem.value]['keyEn'];
                        } else  {
                            textarea.value += keyStorage[elem.value]['keyShiftEn'];
                        }
                    } else if (localStorage.getItem('language') === 'ru') {
                        if (elem.value.slice(0, 3) === "Key" || (["BracketLeft", "BracketRight", "Period", "Comma", "Semicolon", "Quote", "Backquote"].includes(elem.value))) {
                            textarea.value += keyStorage[elem.value]['keyRu'];
                        } else  {
                            textarea.value += keyStorage[elem.value]['keyShiftRu'];
                        } 
                    }
        ...
        if (e.code === button.value) {
            ...
            if (button.value ==="CapsLock") {
                ...
                if (button.classList.value === "") {
                    button.classList.add("activeCapsLock");
                } else if (button.classList.value === "activeCapsLock") {
                    button.classList.remove("activeCapsLock");
                }
